...
interface Producto {
  id: string;
  nombre: string; // Ej: "Vale", "Vale Quincenal"
  montoBase: number;
...
const mockProductos: Producto[] = [
  { id: "PRD-001", nombre: "Vale Tradicional", montoBase: 2000, porcentajeComision: 5, porcentajeInteres: 3, moraDiaria: 50, estado: "Activo" },
  { id: "PRD-002", nombre: "Vale Premium", montoBase: 5000, porcentajeComision: 8, porcentajeInteres: 2.5, moraDiaria: 100, estado: "Activo" },
];

export default function ProductosPage() {
  const [data, setData] = useState<Producto[]>(mockProductos);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [formData, setFormData] = useState({
    nombre: "", montoBase: 0, porcentajeComision: 0, porcentajeInteres: 0, moraDiaria: 0, estado: "Activo"
  });
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <PageHeader 
        ...
        description="El Gerente General habilita y configura los planes de vales base para las distribuidoras."
        breadcrumbs={[{ label: "Operaciones" }, { label: "Productos" }]}
        actions={
          <button 
      ...
      {/* Explicación de Matemáticas */}
      <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 mb-6 text-sm text-blue-800 flex gap-3">
        <svg className="shrink-0 mt-0.5" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>
        <div>
          <strong className="block mb-1">Regla de Negocio: Matemáticas por Porcentaje</strong>
          La responsabilidad del pago es de la Distribuidora. Al levantar un producto, se define el porcentaje base de comisión e interés. Cuando una distribuidora sube de nivel (ej. Plata a Platino), estos porcentajes se ajustan dinámicamente según su categoría.
        </div>
      </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Levantar Producto">
        <form onSubmit={handleSave} className="space-y-4">
          <div>
            ...
            <input 
              required type="text" placeholder="Ej. Vale $3,000"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input 
                ...
                className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-400"
                value={formData.montoBase || ""} onChange={(e) => setFormData({...formData, montoBase: Number(e.target.value)})}
              />
            </div>
            <div>
              ...
              <input 
                required type="number" min="0"
                className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-400"
                value={formData.moraDiaria || ""} onChange={(e) => setFormData({...formData, moraDiaria: Number(e.target.value)})}
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-lg">
            <div>
              ...
              <input 
                ...
                className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-400"
                value={formData.porcentajeComision || ""} onChange={(e) => setFormData({...formData, porcentajeComision: Number(e.target.value)})}
              />
            </div>
            <div>
              ...
              <input 
                required type="number" min="0" max="100" step="0.1"
                className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-400"
                value={formData.porcentajeInteres || ""} onChange={(e) => setFormData({...formData, porcentajeInteres: Number(e.target.value)})}
              />
            </div>
          </div>
          ...
          <div className="mt-6 flex justify-end gap-3 pt-4 border-t border-slate-100">
            ...
            <button type="submit" className="px-4 py-2 text-sm font-semibold text-white bg-primary hover:bg-primary-600 rounded-lg transition-colors shadow-sm">
              Crear Producto
            </button>
          </div>
        </form>
